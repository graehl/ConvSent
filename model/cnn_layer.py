...
def encoder(tparams, layer0_input, filter_shape, pool_size,
    ...
    conv_out = theano.tensor.nnet.conv2d(input=layer0_input, filters=tparams[_p(prefix,'W')],
                            filter_shape=filter_shape)

    conv_out_tanh = tensor.tanh(conv_out + tparams[_p(prefix,'b')].dimshuffle('x', 0, 'x', 'x'))
    output = pool.pool_2d(input=conv_out_tanh, ws=pool_size, ignore_border=True)
